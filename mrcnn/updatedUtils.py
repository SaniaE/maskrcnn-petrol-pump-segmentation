...
def imageSplit(IMAGE_DIR):
    ...
    with open(os.path.join(IMAGE_DIR, jsonFile), "r") as read_file:
        data = json.load(read_file)
    keys = list(data.keys())

    try:
        sp = input("Enter ratio of train/validation/test data split(eg. 70:20:10)= ")
        trainRatio = int(float(sp.split(":")[0]))
        validationRatio = int(float(sp.split(":")[1]))
        testRatio = int(float(sp.split(":")[2]))

        while trainRatio!= 0 and validationRatio!= 0 and testRatio!= 0 and (trainRatio+validationRatio+testRatio) != 100:
            print("\nTry again")
            sp = input("Enter ratio of train/validation/test data split(eg. 70:20:10)= ")
            trainRatio = int(float(sp.split(":")[0]))
            validationRatio = int(float(sp.split(":")[1]))
        ...
        print("\nRestart with proper values")
        
    n =  len(keys)
    nTrain = int(trainRatio*n/100) 
    nValidation = int(validationRatio*n/100) 
    nTest = int(testRatio*n/100) 
    nTrain = nTrain + n - (nTrain+nValidation+nTest) 
    print("Total Images =",n)
    print("No:of Images in Training Set =",nTrain)
    print("No:of Images in Validation Set =",nValidation)
    print("No:of Images in Testing Set =",nTest)  

    train_data=[]
    test_data=[]
    validation_data=[]
    while len(train_data) != (nTrain) :
        i = random.randint(0,len(keys)-1)
        train_data.append(keys[i])
        keys.pop(i)

    while len(validation_data) != (nValidation) :
        i = random.randint(0,len(keys)-1)
        validation_data.append(keys[i])
        keys.pop(i)
    ...
    save_dir = os.path.join(IMAGE_DIR,"train")
    if not os.path.exists(save_dir):
        os.makedirs(save_dir)
    trainJSON = filterByKey(train_data)
    for i in range(len(train_data)):
        f = trainJSON[list(trainJSON.keys())[i]]['filename']
        shutil.move(os.path.join(IMAGE_DIR,f), os.path.join(save_dir,f))       
    with open(os.path.join(save_dir,'via_region_data.json'), 'w') as outfile:
        json.dump(trainJSON, outfile)

    save_dir = os.path.join(IMAGE_DIR,"test")
    if not os.path.exists(save_dir):
        os.makedirs(save_dir)
        
    testJSON = filterByKey(test_data)
    for i in range(len(test_data)):
        f = testJSON[list(testJSON.keys())[i]]['filename']
        shutil.move(os.path.join(IMAGE_DIR,f), os.path.join(save_dir,f))
        
    with open(os.path.join(save_dir,'via_region_data.json'), 'w') as outfile:
        json.dump(testJSON, outfile)

    save_dir = os.path.join(IMAGE_DIR,"val")
    if not os.path.exists(save_dir):
        os.makedirs(save_dir)
        
    validationJSON = filterByKey(validation_data)
    for i in range(len(validation_data)):
        f = validationJSON[list(validationJSON.keys())[i]]['filename']
        shutil.move(os.path.join(IMAGE_DIR,f), os.path.join(save_dir,f))
        
    with open(os.path.join(save_dir,'via_region_data.json'), 'w') as outfile:
        json.dump(validationJSON, outfile)

    os.remove(os.path.join(IMAGE_DIR,"via_region_data.json"))
